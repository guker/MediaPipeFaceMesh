...
with mp_face_detection.FaceDetection(
    ...
    while cap.isOpened():
        ...
        if roi_from_prev_frame:
            face_info = {}
            face_info['score'] = face_score
            # 这里使用tracker方式，与官方效果相近，在未滤波的情况下，视频前后一致性较好。
            # 计算上一帧的landmarks的最小外接矩形
            # 具体参看如下:
            # https://github.com/google/mediapipe/blob/v0.8.7/mediapipe/modules/face_landmark/face_landmark_landmarks_to_roi.pbtxt
            # https://github.com/google/mediapipe/blob/v0.8.7/mediapipe/calculators/util/landmarks_to_detection_calculator.cc
            ...
            face_info['bbox'] = [xmin, ymin, xmax, ymax]
            target_angle = 0
            x0, y0 = landmarks[33][:2]
            x1, y1 = landmarks[263][:2]
            rotation = target_angle - math.atan2(-(y1 - y0), x1 - x0)
            face_info['rotation'] = normalize_radians(rotation)
            bbox_w = xmax - xmin
            bbox_h = ymax - ymin
            face_cx = xmin + bbox_w * 0.5
            face_cy = ymin + bbox_h * 0.5
            ...
            cv2.polylines(image, [face_info['face_pos'].astype(np.int32).reshape((-1, 1, 2))], True,
                          (0, 255, 0), 2)
            # 计算仿射矩阵，类似上面的操作
            ...
            interpreter.set_tensor(interpreter.get_input_details()[0]['index'],
                                   np.expand_dims(crop_im, axis=0).astype(np.float32))
            interpreter.invoke()
            ...
            # 第一帧，使用face detection的结果来实现对齐操作，参考如下:
            ...
            if results.detections:
                for detection in results.detections:
                    ...
                    face_info['bbox'] = [bbox.xmin * w, bbox.ymin * h, bbox.xmin * w + bbox.width * w,
                                         bbox.ymin * h + bbox.height * h]
                    ...
                    center = np.concatenate((np.array(face_cx).reshape(-1, 1, 1), np.array(face_cy).reshape(-1, 1, 1)),
                                            1)
                    points = R @ points + center
